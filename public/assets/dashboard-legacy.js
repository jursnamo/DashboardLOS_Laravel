let raw = [], mode = 'date', charts = {}, globalOutliers = [], statusOutliers = {};
    ...
    let importProgressModalInstance = null;
    ...
    async function togglePresentMode() {
        ...
        if (btn) {
            btn.innerHTML = inMode
                ? '<i class="fal fa-tv me-1"></i>Exit Present'
    ...
    function setStaticSelectOption(id, value) {
        const el = document.getElementById(id);
        if (!el) return;
    ...
    function applyApiMappingToSelects() {
        setStaticSelectOption('mId', 'app_id');
        setStaticSelectOption('mSeg', 'segment');
        setStaticSelectOption('mPurp', 'purpose');
        setStaticSelectOption('mLimit', 'approved_limit');
        ...
        setStaticSelectOption('mStat', 'status_flow');
    }
    ...
    function updateImportProgress(percent, totalRows, label) {
        ...
        const loadedRows = Math.max(0, Math.round((safePercent / 100) * (totalRows || 0)));
        ...
        if (text) {
            ...
            text.textContent = `${prefix}${safePercent}% dari ${(totalRows || 0).toLocaleString()} data (${loadedRows.toLocaleString()} / ${(totalRows || 0).toLocaleString()})`;
        }
    }
    ...
    function showImportProgressModal(totalRows) {
        ...
        if (!importProgressModalInstance) {
            importProgressModalInstance = new bootstrap.Modal(modalEl, {
                backdrop: 'static',
                keyboard: false,
                focus: true
            });
        ...
        const hint = document.getElementById('importProgressHint');
        if (hint) hint.textContent = 'Mohon tunggu, proses sedang berjalan.';
        updateImportProgress(0, totalRows, 'Import');
        importProgressModalInstance.show();
    }

    function startImportProgressTicker(totalRows) {
        ...
        importProgressTimer = setInterval(function () {
            if (importProgressValue >= 95) return;
            importProgressValue += 1;
            updateImportProgress(importProgressValue, totalRows, 'Queue');
        }, 1500);
    }

    function finishImportProgress(success, totalRows, importedRows) {
        if (importProgressTimer) {
            clearInterval(importProgressTimer);
            importProgressTimer = null;
        }

        const finalRows = Number.isFinite(importedRows) ? importedRows : totalRows;
        if (success) {
            updateImportProgress(100, totalRows, 'Import selesai');
            const hint = document.getElementById('importProgressHint');
            if (hint) {
                hint.textContent = `Berhasil import ${finalRows.toLocaleString()} data ke database.`;
            }
        } else {
            const hint = document.getElementById('importProgressHint');
            if (hint) {
                hint.textContent = 'Import gagal. Silakan cek file dan coba lagi.';
            }
        }

        setTimeout(function () {
            if (importProgressModalInstance) {
                importProgressModalInstance.hide();
            }
        }, success ? 450 : 350);
    }
    ...
    async function openDashboardMenu() {
        ...
        try {
            ...
            while (true) {
                ...
                setDashboardLoading(true, `Loading dashboard data... ${pct}% (${loaded.toLocaleString()} / ${total.toLocaleString()})`);
    ...
    async function persistImportToDatabase(mapping) {
        // 1) Start import session
        const startResponse = await fetch(DASHBOARD_IMPORT_API, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json'
            ...
            body: JSON.stringify({
                filename: uploadedFileName || 'manual-upload',
                mode: mode,
                mapping: mapping,
        ...
        if (!startResponse.ok) {
            throw new Error(startData.message || 'Gagal memulai import.');
        }

        const batchId = Number(startData.batch_id || 0);
        if (!batchId) {
            throw new Error('Batch ID tidak valid dari server.');
        }

        // 2) Upload rows in chunks (avoid large single request timeout)
        const totalRows = raw.length;
        let uploadedRows = 0;

        for (let i = 0; i < totalRows; i += DASHBOARD_IMPORT_CHUNK_SIZE) {
            const chunk = raw.slice(i, i + DASHBOARD_IMPORT_CHUNK_SIZE);
            const chunkResponse = await fetch(`${DASHBOARD_IMPORT_API}/${batchId}/chunk`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
            ...
            if (!chunkResponse.ok) {
                throw new Error(chunkData.message || 'Gagal upload chunk import.');
            }

            uploadedRows = Number(chunkData.imported_rows || (uploadedRows + chunk.length));
            const pct = totalRows > 0 ? Math.floor((uploadedRows / totalRows) * 100) : 0;
            importProgressValue = Math.max(importProgressValue, pct);
            updateImportProgress(importProgressValue, totalRows, 'Upload');
        ...
        // 3) Finalize
        const finalizeResponse = await fetch(`${DASHBOARD_IMPORT_API}/${batchId}/finalize`, {
            method: 'POST',
            headers: { 'Accept': 'application/json' }
        });
        ...
        if (!finalizeResponse.ok) {
            throw new Error(finalizeData.message || 'Gagal finalisasi import.');
        }
    ...
    document.getElementById('fileIn').addEventListener('change', e => {
        ...
        uploadedFileName = f.name || 'manual-upload';
    ...
    async function processData() {
        ...
        const generateBtn = document.querySelector('#step2 button[onclick*="processData"]');
        const originalBtnHtml = generateBtn ? generateBtn.innerHTML : '';

        isImportingToDb = true;
        if (generateBtn) {
            generateBtn.disabled = true;
            generateBtn.innerHTML = '<i class="fal fa-spinner fa-spin mr-1"></i>Importing...';
        }

        if (!skipPersistImport) {
            const totalRows = raw.length;
            showImportProgressModal(totalRows);
            startImportProgressTicker(totalRows);

            try {
                const done = await persistImportToDatabase(m);
                finishImportProgress(true, totalRows, Number((done && done.imported_rows) || totalRows));
            } catch(err) {
                finishImportProgress(false, totalRows, 0);
                alert(err.message || 'Gagal import ke database.');
                isImportingToDb = false;
                if (generateBtn) {
        ...
        if (appsWithZeroOrNegativeLimit > 0) {
            ...
            limitInfoBadge.innerHTML = `<span class="badge badge-success badge-pill">Only from ${appsWithPositiveLimit.length} apps (${appsWithZeroOrNegativeLimit} apps without limit)</span>`;
        } else {
            limitSubText.innerHTML = `Portfolio Exposure`;
            limitInfoBadge.innerHTML = `<span class="badge badge-success badge-pill">All ${totalApps} apps have limits</span>`;
        }
        ...
        if (appsWithPositiveLimit.length > 0) {
            ...
            avgLimitInfoBadge.innerHTML = `<span class="badge badge-success badge-pill">From ${appsWithPositiveLimit.length} apps with limit > 0</span>`;
        } else {
            avgLimitSubText.innerHTML = `Average Ticket Size`;
            avgLimitInfoBadge.innerHTML = `<span class="badge badge-warning badge-pill">No applications with limit > 0</span>`;
        }
    ...
    function createCharts(e2eData, globalStats, statusAgg, apps, skipSimInit = false) {
        ...
        branchChartInfo.innerHTML = 
            `<span class="badge badge-success badge-pill"><i class="fal fa-info-circle me-1"></i>Top 10 branches by applications with limit > 0 (${totalBranches} total branches)</span>`;
        ...
        distChartInfo.innerHTML = 
            `<span><i class="fal fa-info-circle me-1"></i>The first bucket is now <7 days (not <8 days). There are 6 distribution buckets. Total Limit is calculated only from applications with limit > 0 in each bucket.</span>`;
            ...
            return `
                <tr>
                    ...
                    <td class="text-center">
                        <span class="badge badge-danger badge-pill" onclick="showOutliersModal('status', '${x.stat}')">${x.out}</span>
                    </td>
                </tr>
            `;
        }).join('');
    ...
    function createLoanSizeScatterChart(e2eData, globalStats) {
        ...
        loanSizeLegend.innerHTML = `
            ...
            <div class="mt-2">
                <small class="text-muted"><i class="fal fa-info-circle me-1"></i>X Axis: Loan Size Category (${loanSizeBuckets.map(b => b.label).join(', ')})</small>
            </div>
        `;
    ...
    function getStepPriority(totalTat) {
        ...
        if (s > 5) return { label: 'High', className: 'badge-danger' };
        if (s > 3) return { label: 'Medium', className: 'badge-warning' };
    ...
    function renderManagementActionBoard(stMetrics) {
        ...
        summaryEl.innerHTML = `
            <span class="badge badge-danger me-1">High: ${highCount}</span>
            <span class="badge badge-primary me-1">Impact >5%: ${eligibleRows.length}</span>
            <span class="badge badge-secondary">Total App: ${totalApp.toLocaleString()}</span>
        `;
...
${rows ? `<ul>${rows}</ul>` : '<div>No reduction applied.</div>'}
    ...
    function showSingleAppDetail(app, refreshOnly = false) {
        ...
        if (isOutlier) {
            ...
            modalHeader.querySelector('.modal-title').innerHTML = `<i class="fal fa-exclamation-triangle me-2"></i>Application Detail Outlier`;
            
            alertBox.className = 'alert alert-warning py-2 mb-3';
            alertBox.querySelector('i').className = 'fal fa-exclamation-triangle me-2';
            ...
            showAllBtn.innerHTML = `<i class="fal fa-list-ul me-1"></i>Show All Outliers`;
            ...
            modalHeader.querySelector('.modal-title').innerHTML = `<i class="fal fa-file-alt me-2"></i>Application Detail Normal`;
            
            alertBox.className = 'alert alert-info py-2 mb-3';
            alertBox.querySelector('i').className = 'fal fa-info-circle me-2';
        ...
        if (isOutlier) {
            document.getElementById('singleAppStatus').innerHTML = `<span class="badge badge-danger badge-pill">Outlier</span>`;
            document.getElementById('singleAppTat').className = 'app-detail-value fw-bold text-danger';
        } else {
            document.getElementById('singleAppStatus').innerHTML = `<span class="badge badge-success badge-pill">Normal</span>`;
        ...
        if (app.limit > 0) {
            analysisText += ` | Limit: ${formatIDR(app.limit)} <span class="badge badge-success badge-pill">(INCLUDED in total portfolio)</span>`;
        } else if (app.limit < 0) {
    ...
    function showDetailModal(type, apps, title, headerColor = "#2563eb", infoText = "") {
        ...
        modalHeader.querySelector('.modal-title').innerHTML = `<i class="fal fa-tasks me-2"></i>${title}`;
                ...
                <div class="card border-0 bg-light">
                    <div class="card-body p-3 text-center">
                        ...
                        <div class="small text-muted mt-1">
                            <span class="badge badge-success badge-pill">${appsWithPositiveLimit.length} with limit > 0</span><br>
            ...
            <div class="col-md-3">
                <div class="card border-0 bg-light">
                    <div class="card-body p-3 text-center">
                        ...
                        <div class="small text-muted mt-1">
                            <span class="badge badge-success badge-pill">From ${appsWithPositiveLimit.length} apps</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        tbody.innerHTML = apps.map((app, index) => {
            ...
            if (isLimitPositive) {
                ...
                limitBadge = `<span class="badge badge-success badge-pill">${formatIDR(app.limit)}</span>`;
            } else if (isLimitNegative) {
                limitClass = 'limit-negative';
                limitBadge = `<span class="badge badge-danger">${formatIDR(app.limit)}</span>`;
            } else {
            ...
            if (type !== 'outlier') {
                ...
                statusCell = `<td class="text-center">
                    <span class="badge ${isOutlier ? 'badge-danger' : 'badge-success'}">
            ...
            return `
                <tr class="${rowClass}">
                    <td>${index + 1}</td>
                    <td><span class="badge badge-light border">${app.id}</span></td>
                    <td>${app.branch}</td>
                    <td>${app.purpOriginal || app.purp}</td>
                    <td><span class="badge badge-secondary">${app.seg}</span></td>
    ...
    function showBottleneckDetail(status, refreshOnly = false) {
        ...
        document.getElementById('slaImpactTable').innerHTML = rowsWithData.length ? rowsWithData.map(r => `
            <tr class="${r.priority === 'High' ? 'table-danger' : (r.priority === 'Medium' ? 'table-warning' : '')}">
                ...
                <td class="text-end">${r.breachCount}/${r.cnt} (${r.breachPct.toFixed(1)}%) ${r.lowSample ? '<span class="badge badge-secondary ms-1">Low Sample</span>' : ''}</td>
                <td class="text-center"><span class="badge ${r.priorityClass}">${r.priority}</span></td>
            </tr>
